...
{
	*new_x = GemSize + size * x + size / 2;
	*new_y = GemSize + size * y + size / 2;
	return;
}
...
void FieldManager::FixField()
{
	...
	for (i = FieldSize - 1; i >= 0; i--)
	{
		for (int j = 0; j < FieldSize; j++)
		{
			if (GemsField[i][j] == NULL)
			{
				ToDispCoords(-size/16, j, &new_x, &new_y);
...
void FieldManager::DeleteGem(Gem* gem)
{
	...
	{
		int i = 0;
		BonusType new_type = (BonusType)(rand() % 2);
		...
		if (new_type == bomb)
			...
			BonusMass[i] = new Bomb(gem->display_x, gem->display_y, gem->mat_x, gem->mat_y, bomb);
		else if(new_type == colorchange)
			BonusMass[i] = new ColorChange(gem->display_x, gem->display_y, gem->mat_x, gem->mat_y, colorchange);
